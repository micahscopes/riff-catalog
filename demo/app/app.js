...
const CH = [
  {
    nav: "start",
    kicker: "the one idea",
    title: "Sameness is a spectrum",
    lede: "Little of what a compiler emits is new; how strict sameness should be is yours to set.",
    body: `<p>Same in every byte, or same once you ignore the names, constants, and types? Each answer is a <em>facet</em>. You turn the dial next.</p>`,
  },
  {
    nav: "facets",
  ...
  },
  {
    ...
    lede: "A short motif and a few variants, fingerprinted live by the same engine: turn the dial to see which count as the same (transpose-proof intervals, rhythm alone, the bare note set), and press play to hear each one.",
  ...
  },
  {
    ...
    lede: "Exact match catches verbatim copies, but real forks edit the function (a dropped modifier, a hand-rolled forwarder); Sourcify, a text search, and exact match all miss them, and matching the shape's subtrees does not.",
  ...
  },
  {
    ...
    lede: "riffcat lives entirely on the syntactic side, a lattice of facets, each a content address; forgetting more moves it toward meaning but never arrives, and does not aim to: its job is to be the substrate a semantic claim pins to.",
    body: `<facet-lattice></facet-lattice>`,
  },
  { nav: "prove it", kicker: "the seat we leave open", title: "A match is a candidate, not a proof", lede: "A shape match is a candidate, not a verdict: riffcat localizes the matching subtree and writes the proof obligation, leaving the verdict and attested-by columns empty for a named verifier. Pick a tool to see what it would have to discharge.", body: `<verdict-seat></verdict-seat>` },
  {
    nav: "anchors",
    kicker: "the heart of the framing: a fact rides an address",
    title: "How a fact rides an address",
    lede: "Pin a fact to a facet address and it transports, free, to every shape sharing the address; it stays sound only while the anchor keeps every dimension the fact depends on. Slide the anchor coarser and watch exactly where it would wrongly travel.",
    body: `<anchor-transport></anchor-transport>`,
  },
  { nav: "prior art", kicker: "the primitive is older than us", title: "Where this idea comes from", lede: "Three communities arrived at the same move on their own: normalize a structure, then address it by its content. Music theory in 1973, a content-addressed Lean kernel, a verified Lean compiler for the EVM. The hash is not what is new here. The faceted, explainable query is.", body: `<prior-art></prior-art>` },
  {
    nav: "the proofs check",
    kicker: "one rung up: the map itself is proved",
    title: "The normal form, proved",
    lede: "A Lean 4 development, kernel-checked, proves the prime form riffcat lands on really is the canonical representative of its transpose-and-invert class; the proof is content-addressed and registered as a claim on the one facet where it stays sound. Hover the badge to read each part honestly.",
  ...
  },
  {
    ...
    lede: "Eight chords, parsed from notation and fingerprinted live in your browser, land at the set-class stop on the published catalog, A/B letters and all: the only fact riffcat is handed is Forte's name for a prime form, every label is derived from structure. Hit invert and A and B trade places while the symmetric chords hold still.",
    body: `<forte-catalog></forte-catalog>`,
  },
  { nav: "interval vector", kicker: "the harmonic signature underneath a chord", title: "A chord's interval signature", lede: "Every chord projects to six interval-class counts, minor second up through the tritone; move it to any key or flip it over and the six numbers hold. Pick a chord; major and minor land on the same signature.", body: "<ivf-fingerprint></ivf-fingerprint>" },
  { nav: "three rungs", kicker: "one chord, three rungs of forgetting", title: "Three rungs of similarity", lede: "Take one chord up the ladder: the literal notes; the tightest packing with transposition forgotten (Tn-type, still 3-11B); the prime form, where inversion folds and 3-11B merges with 3-11A into 3-11. Each rung forgets one more thing.", body: `<three-rungs></three-rungs>` },
  { nav: "two fingerprints", kicker: "two axes, not two rivals", title: "Two kinds of fingerprint", lede: "Sourcify's metadata hash answers \"is this the identical build\" and flips on a single whitespace; the structural fingerprint answers \"is this the same code wearing different clothes\" and is built to hold when the hash moves. Edit the source and watch the axes part.", body: `<metadata-axes></metadata-axes>` },
  {
  nav: "main vs meta",
...
},
  {
    ...
    lede: "riffcat is a small library with a thin CLI, the same engine that ran live in every chapter before this one; we are not asking you to adopt it, we are asking what it should become for the work you do.",
  ...
  },
  {
    ...
    lede: "riffcat does one leg, localization; the compiler can supply provenance from source down to bytecode, and a verifier adjudicates with a proof or a counterexample. Hover a leg for the honest version of what we would need from that team, and the question we cannot answer ourselves.",
    body: `<collab-triangle></collab-triangle>`,
  },
  { nav: "what we sampled", kicker: "the honest numbers, and the ones we owe you", title: "What we measured, and what we didn't", lede: "Every number in this storybook is a Sourcify-floor count over distinct source files, plus one null calibration. None of it is precision or recall against a baseline. Below: exactly what each number is, and the measurement we still owe you.", body: "<sampled-ledger></sampled-ledger>" },
  { nav: "the cheap yes", kicker: "generalizing a fast-path hevm already ships", title: "When structure can skip the solver", lede: "hevm opens its equivalence check with a syntactic cheap yes: if two bytecodes are byte-identical it returns equivalent and never calls a solver. riffcat generalizes that exact-equality check to a facet, so the yes fires on more pairs. Turn the dial and watch where the yes stays sound, where it becomes only a candidate, and the one place it must not fire at all.", body: `<cy-cheap-yes></cy-cheap-yes>` },
  { nav: "seat filled", kicker: "the seat, filled: a Lean proof takes it", title: "A cited proof of equivalence", lede: "The occupant is cited statically: a pinned EquiVM Lean proof (argotorg/EquiVM) that a solc-produced ERC20 runtime is observationally equivalent to a hand-written Solm spec, four-way case split, trust boundary named. Anchored to a facet address it transports to every behavior-complete twin, and the Solm spec is itself a forgetting, so it is a facet too; nothing here runs Lean.", body: `<seat-filled></seat-filled>` },
  { nav: "two instantiations", kicker: "the edge of the method: one generic, two ways", title: "Where structure agrees and types differ", lede: "One generic shape instantiated two ways: at the structure facet one address, one skeleton; keep the type spelling and the addresses part. The gap underneath is honest: this divergence is the syntactic shadow of the instantiation, not the monomorphized form.", body: `<two-instantiations></two-instantiations>` },
  { nav: "the engine checks too", kicker: "the same discipline, turned inward", title: "Checking the engine against itself", lede: "The digest function is written twice: Rust, the engine that ships, and Lean, where the rules are theorems; every ordering is canonical, so both hash one corpus and the bytes must match, exactly, in CI. This is a cited plan over the existing golden substrate, not a live proof: the Lean run waits on the deferred toolchain.", body: `<lockstep-bench></lockstep-bench>` },
  { nav: "the fold", kicker: "how a facet address is made", title: "How the address is computed", lede: "One small unit lowers to a graph. Each node gets a context-free local digest, then the addresses snap in bottom-up: a parent folds its own local content with its children's tree digests, and the whole-graph digest lands last. Step through the fold, then drop a dimension and watch every address change.", body: `<fold-merkle></fold-merkle>` },
  { nav: "provenance", kicker: "the compiler payoff: fold riffcat into origin tracing", title: "Provenance rides along", lede: "Every lowered node keeps an edge back to its source, and the engine addresses the shape both times, live: the address does not move, provenance is queryable payload, not part of the fingerprint the catalog dedups on. The same read reaches across builds, two feature branches of fe or sonatina, or two different compilers of one contract, so where the shape agrees they share an address, and where a pass or a toolchain moves the code the divergence localizes it.", body: `<provenance-rides></provenance-rides>` },
  { nav: "locality runs out", kicker: "the honest edge of a local merkle address", title: "The limits of a local address", lede: "A bottom-up node digest is a function of its own subtree and nothing else (invariant I3). Two real cases push past that: a cycle has no leaf to fold from, and an instantiated shape gets its identity from a context the subtree cannot see. One ships a workaround, one stays an open edge.", body: `<locality-limit></locality-limit>` },
];
...
customElements.define("forte-catalog", class extends HTMLElement {
  ...
  render() {
    ...
    // rung.
    const addrFor = (d) => onClass ? this.shownFp(d).transposition_normal : this.shownFp(d).note_set;
    ...
    this.querySelectorAll("[data-op='invert']").forEach((btn) =>
      btn.addEventListener("click", () => { this.inverted = !this.inverted; this.render(); }));
    // Play the chord as currently shown: inverted pitch classes when invert is on,
    // so the operation is audible, not just labeled.
    this.querySelectorAll(".playbtn").forEach((btn) =>
      btn.addEventListener("click", () => playChord(this.pcsFor(this.data[+btn.dataset.i]))));
...
const TR_RUNGS = [
  ...
  },
  {
    ...
    keeps: "the inside spacing AND the handedness: the A/B-distinguished Tn-type, still apart from its mirror",
  },
  {
    key: "set_class",
    rung: "fold inversion",
    forgets: "the mirror as well: a shape and its inversion share one prime form, so the A and B types merge and the letter drops",
...
customElements.define("three-rungs", class extends HTMLElement {
  async connectedCallback() {
    ...
    try {
      ...
      this.fpInv = JSON.parse(b.fingerprint_pcs(JSON.stringify(this.fp.pitch_classes), true));
  ...
  render() {
    ...
    const rows = TR_RUNGS.map((r, i) => {
      ...
      return `<div class="rungrow ${on ? "on" : ""}" data-rung="${r.key}">`
...
function foldLayout(nodes) {
  ...
  for (const cs of kids.values()) cs.sort((a, b) => (a.order || 0) - (b.order || 0));
  ...
  const VIEW_W = 720, TOP = 60, ROWH = maxDepth > 0 ? Math.min(96, (372 - TOP) / maxDepth) : 0;
  const leaves = [];
  const collect = (n) => {
  ...
  leaves.forEach((n, i) => { n._lx = slot * (i + 1); });
  const place = (n) => {
    const cs = kids.get(n.id);
    if (cs.length === 0) { n.x = Math.round(n._lx); }
    else {
      cs.forEach(place);
  ...
  if (root) place(root);
  for (const n of nodes) { delete n._lx; if (typeof n.x !== "number") { n.x = 360; n.y = 60; } }
}
