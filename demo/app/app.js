...
const CH = [
  ...
  },
  {
    ...
    lede: "Eight named chords, each parsed from notation and fingerprinted live in your browser. At the set-class stop the published catalog appears, standard A/B letters and all: the two major triads collapse to one 3-11B while the minor stays apart as its mirror, 3-11A. Hit invert and A and B trade places; the symmetric chords hold still. The only fact riffcat is handed is Forte's name for a prime form; every label is derived from structure.",
  ...
  { nav: "provenance", kicker: "the compiler payoff: fold riffcat into origin tracing", title: "Where it came from rides along.", lede: "A source expression is lowered to a graph, then every lowered node records where it came from as a provenance edge. The engine addresses the shape both times, live, and the address does not move: provenance is queryable payload, not part of the fingerprint the catalog dedups on. Then take two lowerings that differ by one optimization pass: the engine finds them almost the same shape, and the provenance points at the node that changed. The same read reaches across builds: two feature branches of fe or sonatina, or even two different compilers of one contract, each emits its own artifact map (the lowered graphs with their origin edges), and because the address depends on shape alone you can recognize what stayed the same and localize where the toolchains part.", body: `<provenance-rides></provenance-rides>` },
